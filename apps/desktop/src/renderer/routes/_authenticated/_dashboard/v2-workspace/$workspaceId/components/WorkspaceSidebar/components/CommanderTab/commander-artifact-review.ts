export interface WorkerArtifactCollectionLike {
	status?: string | null;
...
export interface WorkerArtifactSendLike {
	ok?: boolean | null;
	status?: string | null;
...
export interface WorkerArtifactCollectionDecision {
	mode:
		| "artifact-review"
...
export type ReviewGoalMode = "build" | "polish" | "final-review";
...
export interface BudgetedReviewCompletionDecision {
	reviewGoalMode: ReviewGoalMode;
	completionPolicy: ReviewCompletionPolicy;
	turnBudget: number;
	currentTurn: number;
	remainingTurnBudget: number;
	minPolishTurns: number;
	qualityStatus: ReviewQualityStatus;
...
export interface EvaluateBudgetedReviewCompletionInput {
	replyText: string;
	...
	completionPolicy?: ReviewCompletionPolicy | null;
...
export function evaluateBudgetedReviewCompletion(
	input: EvaluateBudgetedReviewCompletionInput,
): BudgetedReviewCompletionDecision {
	...
	if (!input.browserRequestedStop) {
		return {
			reviewGoalMode: remainingTurnBudget > 0 ? "build" : "final-review",
			completionPolicy,
			turnBudget,
			currentTurn,
			remainingTurnBudget,
...
export function classifyArtifactCollection(
	collection: WorkerArtifactCollectionLike | null | undefined,
): WorkerArtifactCollectionDecision {
	if (!collection) {
		return {
...
export function getArtifactSendStopReason(
	sendResult: WorkerArtifactSendLike | null | undefined,
): string | null {
...
export function getArtifactSendAdvisoryReason(
	sendResult: WorkerArtifactSendLike | null | undefined,
): string | null {
	if (!sendResult) return "artifact review send returned no result";
	const status = String(sendResult.status || "").toUpperCase();
...
export function getArtifactReviewReplyStopReason(
	_replyText: string,
): string | null {
	return null;
}

export function getArtifactReviewReplyAdvisoryReason(
	replyText: string,
): string | null {
...
export function getArtifactReviewPendingActionReason(
	replyText: string,
): string | null {
	if (/AI_REFERENCED_FILE\s*:\s*no/i.test(replyText)) {
		return "Browser AI did not reference attached artifacts";
...
export function hasArtifactReviewNextWorkerInstruction(
	workerInstructionText: string,
): boolean {
	return workerInstructionText.trim().length > 0;
}

export function getArtifactReviewMissingNextActionReason(input: {
	replyText: string;
	workerInstructionText: string;
	browserRequestedStop: boolean;
}): string | null {
	...
	if (hasArtifactReviewNextWorkerInstruction(input.workerInstructionText)) {
		return null;
	}
	return "Browser AI artifact review missing STOP or next Worker instruction";
}

export function summarizeArtifactSendResult(
	sendResult: WorkerArtifactSendLike,
): string {
