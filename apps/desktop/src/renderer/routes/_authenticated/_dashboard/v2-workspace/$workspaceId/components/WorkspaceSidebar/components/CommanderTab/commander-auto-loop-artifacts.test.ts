...
import {
	classifyArtifactCollection,
	evaluateBudgetedReviewCompletion,
	getArtifactReviewMissingNextActionReason,
	getArtifactReviewReplyAdvisoryReason,
	getArtifactReviewPendingActionReason,
	getArtifactReviewReplyStopReason,
	getArtifactSendAdvisoryReason,
	getArtifactSendStopReason,
	hasArtifactReviewNextWorkerInstruction,
	summarizeArtifactSendResult,
} from "./commander-artifact-review";

describe("commander artifact review routing", () => {
	it("routes attachable Worker-reported artifacts to Browser AI review", () => {
		const decision = classifyArtifactCollection({
			status: "READY",
	...
	it("stops manual review when artifact collection is blocked", () => {
		const decision = classifyArtifactCollection({
			status: "BLOCKED",
	...
	it("records an advisory when Browser AI reply ignored attached files", () => {
		const stopReason = getArtifactSendStopReason({
			ok: true,
			status: "REPLIED",
			attachedFileCount: 1,
		...
		const advisoryReason = getArtifactSendAdvisoryReason({
			ok: true,
			status: "REPLIED",
			attachedFileCount: 1,
			attachmentUiReflected: true,
	...
	it("validates artifact review markers from the full reply while sending extracted worker instructions", () => {
		...
		expect(getArtifactReviewPendingActionReason(fullReply)).toBeNull();
		expect(
			hasArtifactReviewNextWorkerInstruction(extractedWorkerInstruction),
		).toBe(true);
		expect(
			getArtifactReviewMissingNextActionReason({
				replyText: fullReply,
	...
	it("summarizes successful artifact review sends for review status", () => {
		const summary = summarizeArtifactSendResult({
			ok: true,
	...
	it("does not stop on basic completion while polish budget remains", () => {
		const decision = evaluateBudgetedReviewCompletion({
			replyText: [
				"AI_REFERENCED_FILE: yes",
				"判定",
				"STOP",
		...
		expect(decision.reviewGoalMode).toBe("polish");
	...
	it("accepts early STOP only when Browser AI explains ready-candidate quality", () => {
		...
		expect(decision.reviewGoalMode).toBe("final-review");
