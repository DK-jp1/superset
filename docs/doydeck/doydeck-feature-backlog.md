...
| Live readiness smoke pack | Let Doy quickly decide whether the active tab is ready for Browser AI / Worker / Artifact Review. | Open-ended audit found that readiness was spread across many commands and could be misread. | Implemented as docs plus `getLiveReadinessSummary()` read-only Controller command. | done | Done | Existing inventory, Browser AI preflight, Worker readiness, task-run status, Artifact Review commands, and payload budget fields. | Open-ended deep audit. | Aggregate existing checks without sending Browser AI prompts, sending Worker instructions, binding, or attaching. | `getLiveReadinessSummary({ expectedTabId, expectedTitle, requireActiveTabMatch:true })` returns READY / READY_WITH_NOTES / BLOCKED with blockers and next action. |
...
| Destructive guard false positive test pack | Prevent harmless words from blocking Worker instructions. | `remove` in screenshot names and UI copy caused false stops. | Implemented. | done | Done | Existing `commander-safety.test.ts`. | dangerous guard false positive. | Tests cover screenshot path, `removeFromArray`, UI "削除" labels, and true destructive shell command text as advisory findings. | Unit test pass; DoyDeck records risky command text as warning/advisory and does not block manual review. |
...
| Artifact Review package | Collect Worker screenshots/files/report and send them to Browser AI as real attachments. | Browser AI should review the actual result, not only a text summary. | Implemented as Controller commands for manual review. | done | Done | Existing file attachment command and Worker DONE_TAG extraction. | artifact review. | `collectReviewArtifacts()` collects selected files, review screenshots, Worker report metadata, and Worker-reported artifact paths; `sendWorkerReportedArtifactsToBrowserAI()` handles Worker-reported attachable paths before any text-only review. | Dry-run collection returns attachable artifacts; manual artifact send tests cover artifact send, no-artifact fallback, skipped artifact fallback, and blocked collection. |
...
- Further manual artifact review polish, including clearer attach/review summaries after Worker completion.
...
- DB design and broad UX final decisions.
