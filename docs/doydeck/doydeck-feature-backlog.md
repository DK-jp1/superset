...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Memory lifecycle / payload budget | Keep long DoyDeck sessions and multiple task tabs from becoming heavy. | Worker terminal logs, Controller raw output payloads, Browser AI threads, Handoff history, and artifact state can grow over time. | Initial investigation and small caps added; broader lifecycle policy remains open. | P1 | Now/later | Stable Controller status fields and active-tab lifecycle policy. | Memory lifecycle investigation. | Add `maxDiagnosticChars` / `includeRawOutput` options, Handoff size warnings, per-pane terminal cache diagnostics, and inactive-tab polling policy. | Long-session smoke shows status calls stay small, active tab still works, inactive tabs do not run unnecessary 1s polling. |
