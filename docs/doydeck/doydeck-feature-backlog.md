...
| Live readiness smoke pack | Let Doy quickly decide whether the active tab is ready for Browser AI / Worker / Auto Loop / Artifact Review. | Open-ended audit found that readiness was spread across many commands and could be misread. | Implemented as docs plus `getLiveReadinessSummary()` read-only Controller command. | done | Done | Existing inventory, Browser AI preflight, Worker readiness, task-run status, Auto Loop preflight, Artifact Review commands, safety guard tests, and payload budget fields. | Open-ended deep audit. | Aggregate existing checks without sending Browser AI prompts, sending Worker instructions, binding, attaching, or starting Auto Loop. | `getLiveReadinessSummary({ expectedTabId, expectedTitle, requireActiveTabMatch:true })` returns READY / READY_WITH_NOTES / BLOCKED with blockers and next action. |
...
| Destructive guard false positive test pack | Prevent harmless words from blocking Worker instructions. | `remove` in screenshot names and UI copy caused false stops. | Implemented. | done | Done | Existing `commander-safety.test.ts`. | dangerous guard false positive. | Tests cover screenshot path, `removeFromArray`, UI "削除" labels, and true destructive shell command text as advisory findings. | Unit test pass; DoyDeck records risky command text as warning/advisory and does not hard-stop Loop. |
