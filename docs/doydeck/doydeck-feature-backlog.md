...
| Artifact Review Loop package | Collect Worker screenshots/files/report and send them to Browser AI as real attachments. | Browser AI should review the actual result, not only a text summary. | Implemented as Controller commands for collection and send; prompt contract now asks Browser AI to state `AI_REFERENCED_FILE: yes/no`. Auto Loop body still reuses it explicitly. | P1 | Pilot | Existing file attachment command and Worker DONE_TAG extraction. | Artifact review loop. | `collectLoopReviewArtifacts()` collects selected files, review screenshots, Worker report metadata, and Worker-reported artifact paths; `sendWorkerReportedArtifactsToBrowserAI()` and `sendLoopArtifactsToBrowserAI()` attach files and send the review prompt. | Dry-run collection returns attachable artifacts; bounded-loop send attaches supported files and Browser AI can reply with `AI_REFERENCED_FILE: yes`, STOP, or next Worker instruction. |
