...
| DONE_TAG / END_REPORT extraction | Worker report can shrink to idle text or include prompt echo/footer if extraction is weak. | MyGoalist v0.3 sent only 17 chars to Browser AI. | P0 fixed | Fixed by `5b3d1e6d`; validation hardened by `8db323da`. | `commander-worker-report.test.ts` covers prompt echo, idle-only text, placeholder, and structured reports. | Add more fixture cases for END_REPORT footer/noise and FORMAT_INVALID reports. |
...
| Safety / dangerous guard | False positives on `delete/remove/削除` can block harmless UI/file-name text. | MyGoalist screenshot name `remove` triggered a blocker before hardening. | P1 fixed / P2 watch | Source-aware classifier now blocks delete/remove only for actual shell command source. | `commander-safety.test.ts` covers negated sections, push, rm, and reports. | Add explicit tests for screenshot names, camelCase functions, and UI "削除" labels. |
...
| Controller Commands / API-like操作 | Command surface is broad but still lacks task intake and some scoped Browser AI helpers. | Inventory reports 42 commands and 9 missing commands. | P2 watch | `getControllerCommandInventory()` works. | Current Controller smoke returned `commandCount:42`, `missingCommands:9`. | Prioritize `sendTargetDocsReviewToBrowserAI`, Decision Record accessor, and task intake helpers. |
...
1. Add fixture tests for destructive guard false positives:
   `v06-chat-thread-remove.png`, `removeFromArray`, and UI "削除" label text.
2. Add worker report fixture tests for END_REPORT footer/noise and required
   section validation.
3. Add a Browser AI fixture or provider-independent test page so submission
   verification can be tested without ChatGPT/Claude network state.
4. Add a prompt-size/Handoff-size warning before Browser AI sends.
