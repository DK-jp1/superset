...
以下はDoyDeck作業タブにしない、またはDoy確認前に保留する。
...
- DoyDeck外でやる方が早いもの。
- まだ目的が曖昧すぎるもの。
- 依存関係が未確定で、先にDoyの判断が必要なもの。
- push / deploy / destructive / credentialsが主目的のもの。

扱い:

- その場回答で済むものは回答して終わる。
- 曖昧すぎるものは、タブ作成ではなく「確認質問」または「保留候補」にする。
